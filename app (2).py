...
if st.button("📥 Calculer et enregistrer"):
    ...
    df = pd.DataFrame({
        ...
        'Date d'échéance': [date_echeance.strftime('%d/%m/%Y')],
    ...
    st.download_button("⬇️ Télécharger le fichier Excel", data=df.to_excel(index=False), file_name=fichier_excel, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
